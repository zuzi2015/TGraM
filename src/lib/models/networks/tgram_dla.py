...
class DLASeg(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(num_frames - 1):
            ...
            feat_prev = self.base(feat_prev)
            feat_prev = self.dla_up(feat_prev)
            y = []
            for i in range(self.last_level - self.first_level):
                y.append(feat_prev[i].clone())
            self.ida_up(y, 0, len(y))
            feat_prev = y[-1]
            feat_prev = F.interpolate(feat_prev, scale_factor=0.25, mode="bilinear", align_corners=True)
        ...
        # print('\n message 2')
        state = torch.tanh(self.conv_u(torch.cat([feat_inner, message], 1)))
...
def tgram_dlaseg(num_layers, heads, head_conv=256, down_ratio=4):
    model = DLASeg('dla34',#'dla{}'.format(num_layers),
                   heads,
