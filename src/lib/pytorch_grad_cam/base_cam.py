...
class BaseCAM:
    ...
    def forward(self, input_img):
        ...
        return self.model(input_img)[-1]['hm']
    ...
    def forward(self, input_tensor, target_category=None, eigen_smooth=False):
        ...
        output = self.activations_and_grads(input_tensor)[-1]['hm']

        if type(target_category) is int:
            target_category = [target_category] * input_tensor.size(0)

        if target_category is None:
            # print('\noutput:', np.shape(output))
            # print('\nkeys:', output[-1].keys())
            # import sys
            # sys.exit(0)
            target_category = np.argmax(output.cpu().data.numpy(), axis=-1)
            # target_category = np.argmax(output[-1]['hm'].cpu().data.numpy(), axis=-1)
        else:
        ...
        # loss.backward(torch.ones([1, 152, 152, 272]), retain_graph=True)
